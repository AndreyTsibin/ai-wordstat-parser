...
### 🆕 NEW v2.1: Профессиональные улучшения (январь 2025)
...
1. Прочитай config.json и проверь настройки. Допиши "minus_words" если их мало. Допиши "anchor_texts" если нужно.
2. Запусти workflow.py для полного цикла (генерация запросов → парсинг → контент-план).
3. Проверь файл content_plan.md заголовки статей должны выглядеть корректно. Если нет, прочти файл results.md и составь корректные заголовки.
```
...
**📄 queries.txt** — Автоматически сгенерированные поисковые запросы (18-25 штук, включая запросы от конкурентов)
...
**📊 results.csv** 🆕 — Экспорт в CSV для Excel/Key Collector:
...
{
  "parser_settings": {
    "minus_words": ["ванн", "душев", "туалет", "санузл", "сантехник", "кухн", "плитк"]
  }
}
```

Фразы с этими словами будут отфильтрованы, а в отчёте появится: `**Отфильтровано по минус-словам:** 15`
...
**Проблема:** workflow.py генерирует запросы только по шаблонам → упускаются уникальные фразы конкурентов.
