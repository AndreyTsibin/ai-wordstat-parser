...
1. Прочитай config.json и проанализируй настройки:
   - Проверь и допиши "minus_words" (минимум 5-7 слов для фильтрации)
   - Проверь и допиши "anchor_texts" (минимум 3-5 анкоров)
   - Убедись, что все секции заполнены корректно
...
3. Проверь качество результатов:
   - Открой output/content_plan.md и проверь заголовки статей
   - Если заголовки некорректные, прочитай output/results.md и перегенерируй заголовки
   - Убедись, что все файлы созданы в папке output/

Цель: пользователь получает готовые файлы для работы в папке output/
```

**Что произойдёт автоматически:**
1. ✅ AI прочитает вашу бизнес-информацию из `config.json`
2. ✅ Проверит и дополнит минус-слова и анкоры
3. ✅ Сгенерирует 15-20 поисковых запросов → `queries.txt`
4. ✅ Запустит парсинг Яндекс Вордстат API → `output/results.md`
5. ✅ Экспортирует данные в CSV → `output/results.csv`
6. ✅ Проанализирует результаты и создаст контент-план → `output/content_plan.md`

**Результат:** Готовые файлы в папке `output/` с планом из 30-40 статей за ~1 минуту.
...
**📊 output/results.csv** 🆕 — Экспорт в CSV для Excel/Key Collector:
